...
// Copyright (c) 2011-2012 AXACOIN Developers
...
void ThreadIRCSeed2()
{
    ...
    while (true)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #AXACOINTEST0\r");
            Send(hSocket, "WHO #AXACOINTEST0\r");
        } else {
            // randomly join #AXACOIN00-#AXACOIN99
            // network is now over 3k peers , get them to join 50 random channels!
            //            channel_number = 0; 
            int channel_number = 0; //GetRandInt(50);

            Send(hSocket, strprintf("JOIN #AXACOIN%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #AXACOIN%02d\r", channel_number).c_str());
        }
