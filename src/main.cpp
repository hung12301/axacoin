...
uint256 hashGenesisBlock("0xf1e177015ffc1f0ce7e6435aea3a943e74f126ed47d8571a17ec19620ed0eccb");
...
const string strMessageMagic = "AXACOIN Signed Message:\n";
...
{
    // AXACOIN: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    // AXACOIN: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "axa genesis block";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
	    ...
	    assert(block.hashMerkleRoot == uint256("0xf4d57c7574f20a2f58b0f443cc7d660d2cd79f59f4ad58be4a507e81220a2fb6"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xd2, 0xca, 0xa3, 0xe3 };
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("AXACOINMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("AXACOINMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static AXACOINMiner(CWallet *pwallet)
{
    printf("AXACOINMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("AXACOIN-miner");
    ...
    try { loop {
        ...
        printf("Running AXACOINMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("AXACOINMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&AXACOINMiner, pwallet));
}
